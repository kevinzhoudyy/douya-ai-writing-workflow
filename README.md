...
这套工作流是工具，不是模板。不同人的写作习惯、发布平台、内容类型都不一样，Agent 在适配时应根据实际需求做调整：

- 不是每个步骤都必须走完，简单任务可以跳过中间环节
- 审校标准、配图规则、标题格式都可以按你的实际场景修改
- 目录结构不是固定的，你可以增删工作区来匹配自己的内容分类
- 规则文件是起点，最终要长成你自己的样子
- 你甚至可以不用这一套工作流，而是让 AI Agent 以此为参照，自己打造一套属于你的版本
